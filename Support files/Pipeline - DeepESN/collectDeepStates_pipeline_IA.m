function[statesExt] = collectDeepStates_pipeline_IA(esnMajor,esnMinor,inputSequence,nForgetPoints,leakRateOn)    
        ...
        for n = 2:length(inputSequence(:,1))          
            if i == 1
                states{i}(n,:) = feval(esnMajor.reservoirActivationFunction,((esnMinor(i).inputWeights*esnMinor(i).inputScaling)*([esnMinor(i).inputShift inputSequence(n,:)])')+ esnMajor.connectWeights{i,i}*states{i}(n-1,:)'); %n-1
                 ...
                 states{i}(n,:) = feval(esnMajor.reservoirActivationFunction,((esnMinor(i).inputWeights*esnMinor(i).inputScaling)*([inputSequence(n,:) states{i-1}(n,:)])')+ esnMajor.connectWeights{i,i}*states{i}(n-1,:)'); %n-1
            end
        end
