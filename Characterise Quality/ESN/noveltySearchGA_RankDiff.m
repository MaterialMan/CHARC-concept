...
config.popSize =250; %large pop better
config.totalGens = 2000; %num of gens
config.numMutate = 0.05; 
...
for tests = 1:config.numTests
     ...
     clearvars -except config tests storeError figure1 stats_novelty_KQ stats_novelty_MC...
    ...
    for gen = 2:config.totalGens
        ...
        for i = 1:size(esnMinor,2)
            ...
            %mutate nodes
            if round(rand) && ~config.multiActiv 
                for p = randi([1 10])
        ...
        % Store behaviours   
        pop_metrics = [abs(kernel_rank-gen_rank);MC]'; %abs() used but can be removed
       ...
       if mod(gen,config.saveGen) == 0
            ...
            save(strcat('noveltySearch_run',num2str(tests),'_gens',num2str(config.totalGens),'_',num2str(config.maxMajorUnits),'Nres_',num2str(config.maxMinorUnits),'_nSize.mat'),...
                'database_ext','database_esnMajor','database_esnMinor','config','stats_novelty_KQ','stats_novelty_MC','-v7.3');
